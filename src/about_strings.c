...
Test(about_strings, reference_characters)
{
    ...
    */
    ...
    cr_assert_eq(
        '1', string[3], "Gets character at the sum of both numbers");
    ...
    /*
        ...
        *(string + 1) adds 1 to the pointer then
        dereferences it
    */
    cr_assert_neq(*string + 1, *(string + 1), "They are not equal");
    // C + 1 = D
    cr_assert_eq(*string + 1, 'D', "They are equal");
    // String points to C goes to next address then, take that value
    cr_assert_eq('S', *(string + 1), "They are equal");
}
...
Test(about_strings, sizeof_strlen)
{
    ...
    */
    ...
    cr_assert_eq(sizeof(string), 7, "strlen ends counting at \\0");
...
Test(about_strings, copy)
{
    ...
    cr_assert_str_eq("CSE 201", string2, "Only copied pointer, did not copy the \
        string");
    cr_assert_str_eq(string3, string2, "Only copied pointer, did not copy the \
        string");
    cr_assert_str_eq(string1, string2, "Only copied pointer, did not copy the \
        string");
}
